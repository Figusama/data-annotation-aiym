...
class CrossModalFusion(nn.Module):
    ...
    def __init__(self, video_dim=768, keypoint_dim=256, d_model=1024, n_heads=6):
        super().__init__()
...
class DualHead(nn.Module):
    def __init__(self, dim, vocab_size, mil_beta=5.0):
        super().__init__()
        self.ctc_head = nn.Linear(dim, vocab_size + 1)  # +1 for CTC blank
        self.mil_beta = mil_beta

    def forward(self, x):
        # x: (B, T, dim)
        ctc_logits = self.ctc_head(x)              # (B, T, vocab+1)
        gloss_probs = ctc_logits[..., :-1].softmax(dim=-1)  # exclude blank from MIL
        # differentiable max-pool over time via log-sum-exp (per blueprint 2B)
        beta = self.mil_beta
        mil_scores = (1.0 / beta) * torch.logsumexp(beta * gloss_probs, dim=1)  # (B, vocab)
