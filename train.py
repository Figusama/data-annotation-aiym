...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_metrics = run_epoch(model, train_loader, device, vocab_size,
                                   ...
                                   args.ctc_weight, args.mil_weight, optimizer, scaler)
        val_metrics = run_epoch(model, val_loader, device, vocab_size,
                                 ctc_loss_fn, mil_loss_fn,
                                 args.ctc_weight, args.mil_weight, optimizer=None, scaler=None)
        dt = time.time() - t0

        print(f"[epoch {epoch:3d}/{args.epochs}] "
              f"train_loss={train_metrics['loss']:.3f} "
              f"(ctc={train_metrics['ctc_loss']:.3f} mil={train_metrics['mil_loss']:.3f}) | "
              f"val_loss={val_metrics['loss']:.3f} "
              f"(ctc={val_metrics['ctc_loss']:.3f} mil={val_metrics['mil_loss']:.3f}) | "
              f"{dt:.1f}s")
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            print(f"  -> new best val_loss={best_val_loss:.3f}, saved best.pt")
